...
// ---------------------------------------------------------------------------
// สีประจำมื้อ (ใช้ร่วมกับ scan_page)
// ---------------------------------------------------------------------------
...
class _HomePageState extends State<HomePage> {
  ...
  Widget _buildSummaryCard(ColorScheme cs) {
    return StreamBuilder<int>(
      ...
      builder: (context, snap) {
        ...
        final pct = _dailyGoal <= 0 ? 0.0 : (eaten / _dailyGoal).clamp(0.0, 1.0);
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text('วันนี้', style: TextStyle(color: Colors.grey[700])),
                      ...
                      FilledButton(
                        onPressed: () {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(content: Text('เดี๋ยวค่อยต่อระบบแนะนำอาหารนะ 😊')),
                          );
                        },
  ...
  Widget _buildCalorieSummary() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('สรุปแคลอรี่ (วันนี้)', style: TextStyle(fontWeight: FontWeight.bold)),
        const SizedBox(height: 10),
        _mealTile(title: 'มื้อเช้า', icon: Icons.wb_sunny_outlined, type: 'meal', mealType: 'มื้อเช้า'),
        _mealTile(title: 'มื้อเที่ยง', icon: Icons.sunny, type: 'meal', mealType: 'มื้อเที่ยง'),
        _mealTile(title: 'มื้อเย็น', icon: Icons.nightlight_outlined, type: 'meal', mealType: 'มื้อเย็น'),
        _mealTile(title: 'มื้อทานเล่น', icon: Icons.cake_outlined, type: 'meal', mealType: 'มื้อทานเล่น'),
        const SizedBox(height: 12),
        _mealTile(title: 'การออกกำลังกาย', icon: Icons.directions_run, type: 'exercise', mealType: 'การออกกำลังกาย'),
      ],
    );
  }
  ...
  Widget _buildRecentFoods() {
    return Column(
      children: [
        Row(
          children: [
            const Text('RECENT FOODS', style: TextStyle(fontWeight: FontWeight.w800)),
            const Spacer(),
            Text('ล่าสุด', style: TextStyle(color: Colors.grey[700], fontSize: 12)),
          ],
        ),
        ...
        StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
          ...
          builder: (context, snap) {
            ...
            if (docs.isEmpty) return _buildEmptyFoods();
            return _buildFoodsList(docs);
          },
        ),
      ],
    );
  ...
  Widget _buildFoodsList(List<QueryDocumentSnapshot<Map<String, dynamic>>> docs) {
    return Card(
      elevation: 0,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(18)),
      child: Column(
        children: [
          for (int i = 0; i < docs.length; i++) ...[
            ...
            if (i != docs.length - 1) const Divider(height: 1, indent: 14, endIndent: 14),
          ],
        ],
      ),
    );
  ...
  Widget _recentFoodTile(QueryDocumentSnapshot<Map<String, dynamic>> doc) {
    ...
    final calories = (data['calories'] is num) ? (data['calories'] as num).round() : 0;
    final c = _colorOf(mealType);
  ...
  Widget _buildNutritionBreakdown() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('NUTRITION BREAKDOWN', style: TextStyle(fontWeight: FontWeight.w800)),
        const SizedBox(height: 8),
        Card(
          elevation: 0,
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(18)),
          child: const Padding(
            ...
            child: Column(
              children: [
                _MacroRow(label: 'Carbs', leftText: '40%', rightText: '30%', value: 0.40, color: Colors.orange),
                SizedBox(height: 10),
                _MacroRow(label: 'Fats', leftText: '30%', rightText: '30%', value: 0.30, color: Colors.purple),
                SizedBox(height: 10),
                _MacroRow(label: 'Protein', leftText: '30%', rightText: '40%', value: 0.30, color: Colors.green),
              ],
            ),
          ),
        ),
